...
PREFIX_PATTERN = re.compile(
    r"^users/[a-f0-9]{64}/artifacts/[a-f0-9-]{32,64}$"
)
...
def _put_artifact(target, prefix: str, safe_name: str, body: bytes, content_type: str) -> None:
...
def artifact_tool(prefix: str, *, client=None):
    if not FILES_BUCKET_NAME or not PREFIX_PATTERN.fullmatch(prefix):
...
def image_tool(prefix: str, *, client=None, s3_client=None):
    ...
    def generate_image(
        ...
        if (
            ...
            or len(negative_prompt.strip()) > 1024
        ):
        ...
        if (
            ...
            raise RuntimeError("Image generation was filtered or could not be completed")
