...
class FakeBedrock:
    ...
    def invoke_model(self, **request) -> dict:
        ...
        return {
            "body": FakeBody(
                {"images": [base64.b64encode(self.image).decode("ascii")]}
            )
        }
...
def test_artifact_tool_rejects_binary_and_oversized_outputs(monkeypatch) -> None:
    ...
    save = artifacts.artifact_tool(prefix, client=SimpleNamespace(put_object=lambda **_: None))
...
def test_spreadsheet_formula_like_values_remain_plain_text(monkeypatch) -> None:
    ...
    assert [workbook.active.cell(row=index, column=1).data_type for index in range(2, 5)] == [
        "s",
        "s",
        "s",
    ]
