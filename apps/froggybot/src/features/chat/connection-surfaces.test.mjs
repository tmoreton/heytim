...
test('customer connection screens do not expose developer-key entry controls', async () => {
  const sources = await Promise.all([
    ...
    readFile(new URL('./bot-editor.tsx', import.meta.url), 'utf8'),
  ]);
  ...
  assert.match(sources[0], /connectionProviderFamilies/);
  assert.match(sources[0], /family\.providers\.map/);
  ...
  assert.match(sources[3], /connectionProviderToolGroups/);
  assert.match(sources[3], /providerToolGroups\.groups\.map/);
});
