...
class ApiSafetyTests(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        def resource(_service: str):
            return SimpleNamespace(
                Table=lambda name: cls.invite_table if name == "invites" else cls.data_table
            )
    ...
    def test_revoking_a_share_removes_snapshot_pointers_and_signup_access(self) -> None:
        ...
        self.assertIn({"pk": "SHARE#secret-token", "sk": "META"}, self.data_table.deleted)
        self.assertIn(
    ...
    def test_approving_an_interactive_message_queues_exactly_that_turn(self) -> None:
        ...
        self.assertEqual(
            approval_update["ConditionExpression"], "#status = :awaiting"
        )
    ...
    def test_account_identity_is_deleted_after_application_cleanup(self) -> None:
        ...
        self.assertIn({"pk": "CHAT#user-1#bot-1", "sk": "TURN#1"}, self.data_table.deleted)
        self.assertIn({"pk": "PUSH_TOKEN#push-1", "sk": "OWNER"}, self.data_table.deleted)
