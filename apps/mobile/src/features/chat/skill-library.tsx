      ...
      <KeyboardAvoidingView style={styles.page} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
        <View style={styles.header}>
          ...
          {mode === 'list' ? (
            ...
            <Pressable accessibilityRole="button" accessibilityState={{ busy, disabled: busy }} hitSlop={12} disabled={busy} onPress={save}>
        ...
        <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
          {mode === 'list' ? (
            <>
              ...
              {skills.map((skill) => (
                <Pressable accessibilityRole="button" key={skill.id} style={({ pressed }) => [styles.card, pressed && styles.pressed]} onPress={() => open(skill)}>
...
function SkillForm({ draft, tools, onChange }: { draft: SkillDraft; tools: Capability[]; onChange: (value: SkillDraft) => void }) {
  ...
  return (
    <>
      ...
      <TextInput
        accessibilityLabel="Skill short summary"
        style={styles.input}
