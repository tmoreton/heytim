...
def _schedule_values(value: dict, previous: dict | None = None) -> dict:
    ...
    values = {
        "name": _validate_string(
            value.get("name", prior.get("name")), "name", SCHEDULE_NAME_MAX_LENGTH
        ),
        "prompt": _validate_string(
            value.get("prompt", prior.get("prompt")),
            "prompt",
            SCHEDULE_PROMPT_MAX_LENGTH,
        ),
        "frequency": frequency,
    ...
    if frequency == "monthly":
        ...
        if (
            ...
            or not SCHEDULE_DAY_OF_MONTH_MIN <= day_of_month <= SCHEDULE_DAY_OF_MONTH_MAX
        ):
            raise ApiError(
                400,
                "dayOfMonth must be between "
                f"{SCHEDULE_DAY_OF_MONTH_MIN} and {SCHEDULE_DAY_OF_MONTH_MAX}",
            )
