...
def test_patch_boundary_blocks_workflow_and_itself() -> None:
    ...
    with pytest.raises(ValueError, match="cannot modify"):
        validate_patch(patch_for("services/agent-runtime/autofix_agent.py"))
...
def test_only_small_repairs_with_regression_tests_can_auto_merge() -> None:
    safe = patch_for(
        "services/agent-runtime/runtime/frogbot_runtime/streaming.py",
        "services/agent-runtime/tests/test_streaming.py",
    )
    paths = validate_patch(safe)
    assert classify_patch(safe, paths) == (True, [])

    sensitive = patch_for(
        "services/froggybot-api/amplify/functions/auth/session.py",
        "services/froggybot-api/amplify/functions/tests/test_session.py",
    )
...
def test_source_change_without_a_test_requires_review() -> None:
    patch = patch_for("services/agent-runtime/runtime/frogbot_runtime/streaming.py")
