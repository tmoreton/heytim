...
IMAGE_MODEL_ID = os.environ.get(
    "HEYTIM_IMAGE_MODEL_ID", "openai/gpt-image-2.5-sunburst"
)
IMAGE_QUALITY = os.environ.get("HEYTIM_IMAGE_QUALITY", "high")
IMAGE_REQUEST_TIMEOUT_SECONDS = int(
    os.environ.get("HEYTIM_IMAGE_REQUEST_TIMEOUT_SECONDS", "300")
)
IMAGE_MAX_ATTEMPTS = int(os.environ.get("HEYTIM_IMAGE_MAX_ATTEMPTS", "2"))
...
if not re.fullmatch(
    ...
    raise ValueError("HEYTIM_IMAGE_MODEL_ID is invalid")
if IMAGE_QUALITY not in IMAGE_QUALITIES:
    raise ValueError("HEYTIM_IMAGE_QUALITY is invalid")
if not 30 <= IMAGE_REQUEST_TIMEOUT_SECONDS <= 600:
    raise ValueError("HEYTIM_IMAGE_REQUEST_TIMEOUT_SECONDS must be between 30 and 600")
if not 1 <= IMAGE_MAX_ATTEMPTS <= 3:
    raise ValueError("HEYTIM_IMAGE_MAX_ATTEMPTS must be between 1 and 3")
