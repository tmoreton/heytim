...
MANAGED_BROWSER_LOCAL_SESSION = "heytim-private-browser"
...
class PersistentAgentCoreBrowser(AgentCoreBrowser):
    ...
    def __init__(
        self,
        session_name: str,
        managed_session: dict | None = None,
    ...
    @tool(
        name="browser",
        description=(
            AgentCoreBrowser.browser.tool_spec["description"] + MANAGED_BROWSER_GUIDANCE
        ),
    )
    ...
    @tool(
        name="capture_points_screenshot",
        description=(
            "Capture one tightly selected points-price card from the current official airline "
            "or hotel page and save it as a private PNG artifact. First use browser get_text "
            "to verify the selector and exact points value. Never use this for publishers, "
            "blogs, aggregators, search results, full pages, account balances, member IDs, "
            "booking codes, or other personal data. The tool returns an artifactId that can "
            "be embedded in a Gmail HTML draft as cid:<artifactId>."
        ),
    )
    async def capture_points_screenshot(
        ...
        """Save verified first-party points evidence from the active browser page."""
        return await asyncio.get_running_loop().run_in_executor(
            self._executor,
            contextvars.copy_context().run,
            self._capture_points_screenshot,
            sessionName,
            selector,
            filename,
            pointsValue,
    ...
    def _capture_points_screenshot(
        self,
        session_name: str,
        selector: str,
        filename: str,
        ...
        return self._execute_async(
            self._async_capture_points_screenshot(
                session_name,
                selector,
                filename,
                points_value,
    ...
    async def _async_capture_points_screenshot(
        self,
        session_name: str,
        selector: str,
        filename: str,
        ...
        if not self.artifact_prefix:
            raise ValueError("Screenshot artifact storage is unavailable")
...
def _prepare_playwright_driver() -> None:
    ...
    if not os.access(source, os.X_OK):
        try:
            ...
            target = Path(tempfile.gettempdir()) / "heytim-playwright-node"
    ...
    *,
    ...
    if "code_interpreter" in names:
        interpreter = PersistentAgentCoreCodeInterpreter(
            region=AWS_REGION,
            session_name=f"heytim-{session_id}",
    ...
    if "browser" in names:
        ...
        if artifact_prefix:
            tools.append(browser.capture_points_screenshot)
