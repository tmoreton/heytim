...
2. Create a scheduled research group containing Chief and Trend Scout. Keep YouTube Studio in direct chat because its image tool prevents group replies. Set the group's audience, subject, channel identifiers, voice and source watchlist in its notebook. Do not paste credentials into a notebook or prompt.
...
The runtime has an OpenRouter image generator and private artifact storage. YouTube Studio's Thumbnail Director can create polished 1280x720 thumbnails with exact copy plus recent, user-owned image references from the same direct chat. Web search can locate official brand guidance and source pages, while exact marks should come from uploaded official assets rather than a model redrawing them. Because image generation is interactive, YouTube Studio cannot participate in group replies or group schedules. Direct turns require approval for the bot's full tool set, even for research-only requests; a direct bot schedule can run only after Image Generator has been persistently allowed.
