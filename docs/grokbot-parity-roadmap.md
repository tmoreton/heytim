# HeyTim capability-parity roadmap
...
HeyTim already has the core persistent-team model: named bots, scoped memory, reusable skills,
...
but its primary product model remains a creator platform for individual bots. HeyTim can own the
...
- Downloadable text, Markdown, CSV, JSON, and HTML artifacts generated by a HeyTim
...
- Use HeyTim-owned server-side credentials for shared tools instead of asking users for developer keys
...
- Direct-chat private self-skill creation for every bot, restricted to the bot's existing tools and disabled for
  scheduled and group mutations
...
1. **Complete:** deployed through `HeyTimDeploymentRole`; the bootstrap access key was removed after use.
