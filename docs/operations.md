...
The read-only concurrency check is `npm run load:test -- --requests 40 --concurrency 8` from `services/froggybot-api`.
...
The destructive authenticated workflow suite is `npm run workflow:test` from `services/froggybot-api`. It requires an
