...
export async function preparePhotoAttachment(
  ...
  if (
    ...
    && hasMatchingType
    && originalSize > 0
    && originalSize <= constraints.imageMaxBytes
  ) {
    return {
      uri: asset.uri,
      name: originalName,
  ...
  if (largestDimension > constraints.maxPhotoDimension) {
    if (asset.width >= asset.height) context.resize({ width: constraints.maxPhotoDimension });
    else context.resize({ height: constraints.maxPhotoDimension });
  }
  ...
  for (const compress of JPEG_QUALITY_ATTEMPTS) {
    ...
    if (size > 0 && size <= constraints.imageMaxBytes) {
      return {
        uri: result.uri,
        name,
        size,
