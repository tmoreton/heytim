...
import type { BotAction, ChatOverlay } from './chat-overlay';
...
export function useChatActions({
  ...
  const activeDirectTurn = selectedBot
    ? [...chat.messages].reverse().find(
        (message) => message.allowedActions?.includes('cancel'),
      )
    : undefined;
  ...
  const respondToApproval = async (message: Message, always?: boolean) => {
    const action = always === undefined ? 'reject' : always ? 'approveAlways' : 'approveOnce';
    if (!selectedBot || !message.allowedActions?.includes(action)) return;
    try {
      if (always === undefined) {
