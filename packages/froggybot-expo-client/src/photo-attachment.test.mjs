...
const constraints = { imageMaxBytes: 3_750_000, maxPhotoDimension: 1_920 };
...
test('prepares a supported browser photo for upload without changing its bytes', async () => {
  const result = await preparePhotoAttachment(photo(), 1, constraints);
...
test('rejects unsupported browser photo formats before upload', async () => {
  await assert.rejects(
    preparePhotoAttachment(photo({ fileName: 'launch.heic', mimeType: 'image/heic' }), 1, constraints),
    /supported image/,
  );
});
...
test('rejects oversized browser photos before requesting an upload ticket', async () => {
  await assert.rejects(
    preparePhotoAttachment(photo({ fileSize: 3_750_001 }), 1, constraints),
    /too large/,
  );
});
