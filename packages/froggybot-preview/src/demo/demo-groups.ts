import type { Attachment, Bot, Group, GroupDraft, Message } from '@froggybot/contracts';
import { demoDecisionsForGroup } from '../demo-decisions';
import { withDemoGroupActions, withDemoMessageActions } from '../demo-contract';

import { cloneGroup, demoState } from './demo-state';

export const demoGroupMessages = (groupId: string): Message[] => {
  const savedDecisionIds = new Set(demoDecisionsForGroup(groupId).map((decision) => decision.sourceMessageId));
  return (demoState.groupMessages.get(groupId) ?? []).map((message) => withDemoMessageActions(message, savedDecisionIds));
};
...
export const demoSaveGroup = (draft: GroupDraft, groupId?: string): Group => {
  ...
  const group: Group = withDemoGroupActions({
...
export const demoJoinGroup = (_token: string): Group => withDemoGroupActions(cloneGroup(demoState.groups[0]));
