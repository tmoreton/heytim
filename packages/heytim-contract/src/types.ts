...
export type Bot = {
  ...
  allowedActions?: Array<'edit' | 'documents' | 'browser' | 'schedule' | 'share' | 'clear' | 'delete'>;
};
...
export type Message = {
  ...
  status:
    ...
    | 'cancelled'
    | 'error';
  allowedActions?: Array<'reject' | 'approveOnce' | 'approveAlways' | 'cancel' | 'saveDecision'>;
};
...
export type GroupMember = {
  ...
  name: string;
  role: 'owner' | 'member';
  allowedActions?: Array<'remove' | 'leave'>;
};
...
export type Group = {
  ...
  isOwner: boolean;
  allowedActions?: Array<'view' | 'edit' | 'share' | 'schedule' | 'delete' | 'viewMemory' | 'manageMemory'>;
  members: GroupMember[];
  bots: GroupBot[];
...
export type Capability = {
  ...
  name: string;
  description: string;
  provider?: 'stan' | 'heytim' | 'agentcore' | 'agentcore-gateway' | string;
...
export type AppConstraints = {
  botNameMaxLength: number;
  botTaglineMaxLength: number;
  botPromptMaxLength: number;
  groupNameMaxLength: number;
  groupMemoryMaxLength: number;
  messageMaxLength: number;
  scheduleNameMaxLength: number;
  schedulePromptMaxLength: number;
  scheduleDayOfMonthMin: number;
  scheduleDayOfMonthMax: number;
