...
Apple verification, routine checks, and TestFlight builds run on the repository-scoped Mac mini runner; non-Apple
suites are selected by changed path so an unrelated edit does not start every job. The maintained CI and
repository-boundary rationale is in [`docs/deployment-architecture.md`](docs/deployment-architecture.md).
