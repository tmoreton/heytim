...
def group_bots(items: list[dict]) -> list[dict]:
    ...
    return sorted(
        ...
        key=lambda item: (str(item.get("name", "")).lower(), str(item.get("botId", ""))),
    )
...
def group_round_step(replies: Any, index: Any) -> tuple[dict, bool]:
    ...
    if isinstance(index, bool) or not isinstance(index, int) or not 0 <= index < len(replies):
        raise ValueError("Group agent round position is invalid")
    reply = replies[index]
    if not isinstance(reply, dict):
        raise ValueError("Each group agent round reply must be an object")
