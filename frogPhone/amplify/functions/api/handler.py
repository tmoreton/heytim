...
def _require_group_member(user_id: str, group_id: str, *, owner: bool = False) -> tuple[dict, list[dict]]:
...
def _update_group(user_id: str, group_id: str, value: dict) -> dict:
    ...
    current_bot_ids = {item["botId"] for item in items if item.get("entity") == "GROUP_BOT"}
...
def _join_group(user_id: str, display_name: str, token: str) -> dict:
    ...
    invite = table.get_item(Key={"pk": f"GROUP_INVITE#{token}", "sk": "META"}, ConsistentRead=True).get("Item")
        ...
        messages.append(
            {
                ...
                for key, value in {
                    ...
                    "isMine": item.get("authorType") == "user" and item.get("authorId") == user_id,
                    "text": item.get("text", ""),
...
def _send_group_message(user_id: str, display_name: str, group_id: str, value: dict) -> dict:
...
def _bootstrap(user_id: str) -> dict:
    ...
    if not bots:
        ...
        bots = migrated
    return {"bots": bots, "groups": _list_groups(user_id), "tools": catalog.list_tools(), "skills": catalog.list_skills(user_id)}
...
def handler(event: dict, _context: Any) -> dict:
    try:
        ...
        if method == "PUT" and path.startswith("/groups/") and "/members/" not in path:
            return _response(200, _update_group(user_id, params.get("groupId", ""), _body(event)))
        if method == "GET" and path.startswith("/groups/") and path.endswith("/messages"):
            return _response(200, {"messages": _list_group_messages(user_id, params.get("groupId", ""))})
        if method == "POST" and path.startswith("/groups/") and path.endswith("/messages"):
            return _response(202, _send_group_message(user_id, display_name, params.get("groupId", ""), _body(event)))
        ...
        if method == "POST" and path.startswith("/group-invites/") and path.endswith("/join"):
            return _response(201, _join_group(user_id, display_name, params.get("token", "")))
        if method == "DELETE" and "/members/" in path:
            return _response(200, _remove_group_member(user_id, params.get("groupId", ""), params.get("memberId", "")))
